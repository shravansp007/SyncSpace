...
import { BehaviorSubject, Subject, catchError, combineLatest, map, of, takeUntil, tap } from 'rxjs';
...
export class DashboardService {
  ...
  constructor(
    ...
    this.loadStatsOnce();
    this.loadHealthOnce();
    this.trackRealtimeSignals();
  }

  private loadStatsOnce(): void {
    combineLatest([
  ...
  private loadHealthOnce(): void {
    const startedAt = performance.now();

    this.http.get(`${environment.apiBaseUrl}/`, { responseType: 'text' }).pipe(
      map(() => {
  ...
  private trackRealtimeSignals(): void {
